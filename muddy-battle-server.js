...
function renderRoom(player) {
  ...
  const enemyText = enemy && enemy.hp > 0
    ? `Enemy: ${enemy.name} (${enemy.hp}/${enemy.maxHp} HP)`
    : 'Enemy: none';
...
function defeatEnemy(player, enemy) {
  ...
  scheduleRespawn(worldState, roomId, () => {
    const respawned = cloneEnemy(roomSpawns[roomId]);
    respawned.stats = {
      offense: respawned.offense,
      defense: respawned.defense,
      speed: respawned.speed,
      ki: respawned.powerLevel,
      strength: respawned.powerLevel,
    };
    respawned.hp = respawned.currentHp;
    respawned.maxHp = respawned.maxHp || respawned.hp;
    respawned.kiPool = respawned.currentKi;
    respawned.maxKiPool = respawned.maxKiPool || respawned.kiPool;
    ensureCombatState(respawned);
    return respawned;
  }, 15000);
...
function enemyCounterattack(player, enemy) {
  ...
  if (!check.ok) {
    return `${enemy.name} hesitates and cannot act.`;
  }
...
function handleGameCommand(player, line) {
  ...
  switch (normalized) {
    case 'help':
      return 'Commands: help, look, stats, fight, punch, kick, blast, beam, charge, meditate, scan, north/south/east/west, quit';
    case 'look':
      return renderRoom(player);
    case 'stats':
...
const server = net.createServer((socket) => {
  ...
  writeLine(socket, `${config.motd} [battle build]`);
...
server.listen(config.port, config.host, () => {
  console.log(`Muddy battle server listening on ${config.host}:${config.port}`);
});
