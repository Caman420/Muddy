...
function handleUnfollow(player) {
  clearFollowTarget(worldState, player.id);
...
function handleGameCommand(player, line) {
  ...
  switch (cmd.toLowerCase()) {
    case 'help': return 'Commands: help, look, stats, who, say <msg>, scan, pvp [on|off], follow <party member>, unfollow, party invite <name>, party accept <name>, party leave, party disband, party list, punch <target>, kick <target>, blast <target>, beam <target>, north/south/east/west, charge, meditate, quit';
...
const server = net.createServer((socket) => {
  ...
  writeLine(socket, `${config.motd} [coop build]`);
...
server.listen(config.port, config.host, () => {
  console.log(`Muddy coop server listening on ${config.host}:${config.port}`);
});
