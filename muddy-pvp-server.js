...
function otherPlayersText(viewer) {
  const others = getRoomPlayerIds(worldState, viewer.roomId)
    ...
    .map((p) => `${p.name}${p.pvpEnabled ? '' : ' [safe]'}`);
...
function handleScan(player) {
  ...
  const players = getRoomPlayerIds(worldState, player.roomId)
    ...
    .map((p) => `${p.name} | PL ${p.stats.powerLevel} | HP ${p.hp}/${p.maxHp} | PvP ${p.pvpEnabled ? 'on' : 'off'}`);
  const lines = [];
...
function handleGameCommand(player, line) {
  ...
  switch (cmd.toLowerCase()) {
    case 'help': return 'Commands: help, look, stats, who, say <msg>, scan, pvp [on|off], punch <target>, kick <target>, blast <target>, beam <target>, north/south/east/west, charge, meditate, quit';
    ...
    case 'pvp': return handlePvpToggle(player, target);
    case 'punch':
    case 'fight':
...
const server = net.createServer((socket) => {
  ...
  writeLine(socket, `${config.motd} [pvp build]`);
...
server.listen(config.port, config.host, () => {
  console.log(`Muddy PvP server listening on ${config.host}:${config.port}`);
});
