...
function patchServer(root) {
  ...
  const importMarker = "const { ensureProgression, applyRacePerk, grantLevelSkillPoint, learnTechnique, renderProgression } = require('./src/world/progression');\n";
  const importBlock = "const { ensureQuestState, listAvailableQuests, acceptQuest, recordQuestKill, turnInQuest, renderQuestLog } = require('./src/world/quests');\nconst { ensureTrainerState, renderTrainerMenu, trainStat } = require('./src/world/trainers');\n";
  content = insertOnce(content, importMarker, importBlock, 'after');
